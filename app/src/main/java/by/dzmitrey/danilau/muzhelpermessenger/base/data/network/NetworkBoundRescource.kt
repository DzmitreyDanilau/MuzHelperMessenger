...
constructor(
    ...
    private val stateEvent: Result.State,
    ...
    val result: Flow<DataState<ViewState>> = flow {
        ...
        when (apiResult) {
            ...
            is ApiResult.NetworkError -> {
                emit(buildError(ErrorsConstants.NETWORK_ERROR, UIComponentType.Dialog(), stateEvent))
            }

            is ApiResult.Success -> {
                if (apiResult.value == null) {
                    emit(buildError(ErrorsConstants.ERROR_UNKNOWN, UIComponentType.Dialog(), stateEvent))
                } else {
    ...
    private suspend fun returnCache(markJobComplete: Boolean): DataState<ViewState> {
        ...
        return object : CacheResponseHandler<ViewState, CacheObj>(response = cacheResult, stateEvent = jobCompleteMarker) {
